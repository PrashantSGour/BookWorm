...
public class CartDetailsController {
    ...
    @PostMapping("/add")
    public ResponseEntity<CartDetails> addProductToCart(@RequestParam int customerId,
                                                         @RequestParam int productId,
                                                         @RequestParam int quantity,
                                                         @RequestParam int rentNoOfDays,
                                                         @RequestParam String transType) {

        // Check if the customer has a cart_master
        Optional<CartMaster> cartMasterOptional = cartMasterRepository.findByCustomerId(customerId);
        ...
        CartDetails cartDetails = cartDetailsService.addProductToCart(customerId, productId, quantity, rentNoOfDays, transType);
